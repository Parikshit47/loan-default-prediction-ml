...
try:
    ...
    from models.base import BaseModelPredictor
    from schemas import LoanPredictionInput, PredictionResponse


def resolve_model_path(filename: str) -> Path:
    """
    Searches multiple standard locations for the given model pickle filename.
    Works seamlessly whether running locally, in subdirectories, or in Vercel Serverless runtime.
    """
    current_dir = Path(__file__).resolve().parent
    candidates = [
        current_dir / "saved_models" / filename,
        current_dir / filename,
        current_dir.parent / "models" / "saved_models" / filename,
        current_dir.parent.parent / "notebooks-containing-models" / filename,
        Path.cwd() / "backend" / "models" / "saved_models" / filename,
        Path.cwd() / "notebooks-containing-models" / filename,
        Path.cwd() / filename,
    ]
    for candidate in candidates:
        if candidate.is_file():
            return candidate
    return current_dir / "saved_models" / filename
...
class DecisionTreePredictor(BaseModelPredictor):
    ...
    def __init__(self, pkl_path: str = None):
        ...
        if pkl_path is None:
            self.pkl_path = resolve_model_path("DecisionTreeModel.pkl")
        else:
